...
function Test() {
  ...
  const settings = {
    ...
    destination: "http://localhost:5000/api/upload/007",
    destinationheaders: {
      Method: "POST",
    },
  ...
  const scanSetParams = async () => {
    try {
      ...
      const result = await fetch(
        `http://localhost:5000/api/ocrData`,
        {
          method: "POST",
          headers: {
          ...
          <div key={"normal images"}>
            {fileNames?.map((file, index) => {
              return (
                  ...
                  <img
                    key={index+1}
                    src={`http://localhost:5000/api/images/${file.filename.replace(
                      ".tif",
                      ".jpg"
                    )}`}
            ...
            {fileNames?.map((file, index) => {
              return (
                <>
                  ...
                  <img key={index} src={`http://localhost:5000/api/B&Wimages/${file.filename.replace(".tif",".jpg"
                    )}`}
                    className="w-[750px] h-[300px]"
                  ></img>
                </>
