...
function App() {
  ...
  const settings = {
    ...
    destination: "http://34.47.233.91:5000/upload/007",
    destinationheaders: {
      Method: "POST",
    },
  ...
  const scanSetParams = async () => {
    try {
      ...
      const result = await fetch(
        `http://34.47.233.91:5000/ocrData`,
        {
          method: "POST",
          headers: {
          ...
          <div key={"normal images"}>
            {fileNames?.map((file, index) => {
              return (
                  ...
                  <img
                    key={index+1}
                    src={`http://34.47.233.91:5000/images/${file.filename.replace(
                      ".tif",
                      ".jpg"
                    )}`}
            ...
            {fileNames?.map((file, index) => {
              return (
                <>
                  ...
                  <img key={index} src={`http://34.47.233.91:5000/B&Wimages/${file.filename.replace(".tif",".jpg"
                    )}`}
                    className="w-[750px] h-[300px]"
                  ></img>
                </>
